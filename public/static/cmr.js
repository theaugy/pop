...
BackendImpl.prototype.TagSongs = function(tag, songs, cb) {
   var args = ['tag', tag];
   songs.forEach((s) => { args.push("path"); args.push(s.path); s.tags.push(tag); });
...
BackendImpl.prototype.UntagSongs = function(tag, songs, cb) {
   var args = ['tag', tag];
   songs.forEach((s) => { args.push("path"); args.push(s.path); arem(s.tags, tag); });
...
BackendImpl.prototype.SearchForTag = function(tag, callback) {
...
BackendImpl.prototype.LoadAugysPhone = function() {
   this.request("loadAugysPhone", (response) => {
      console.log(response);
   });
}

BackendImpl.prototype.LoadStellasPhone = function() {
   this.request("loadStellasPhone", (response) => {
      console.log(response);
   });
}
